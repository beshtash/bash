...
# Make sure that it be executed with superuser(root) privileges.

if [[ "${UID}" -ne 0 ]]; then echo "Please use sudo or be root user." >&2; exit 1; fi
...
# If the user doesn't supply at least one argument, give the help.
if [[ ${#} -lt 1 ]]
then
  usage
fi
...
do
  ...
  if [[ "${USERID}" -lt 1000 ]] 
  then
    echo "Fefusing to remove the ${USERNAME} account with UID ${USERID}." >&2
      exit 1
  fi
  ...
  # Archive the user's home directory and move it into the ARCHIVE_DIR
    ...
    ARHIVE_FILE="${ARCHIVE_DIR}/${USERNAME}.tgz"
    if [[ -d "$HOME_DIR" ]]
    then 
